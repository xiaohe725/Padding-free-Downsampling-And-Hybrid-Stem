...
class ConvBNActivation(nn.Sequential):
    def __init__(self,
        ...
        if in_planes!=3:
            super(ConvBNActivation, self).__init__(norm_layer(in_planes),
...
class StemBlock(nn.Module):
    def __init__(self, inp=3, num_init_features=32):
        ...
        self.stem_1 = ConvBNActivation(inp, num_init_features, 3, 1, 1)
        self.stem_1_1 = ConvBNActivation(num_init_features, num_init_features, 3, 1, 1,groups=num_init_features)
        #self.stem_2a = ConvBNActivation(num_init_features, int(num_init_features / 2), 1, 1, 0)
        #self.stem_2b = ConvBNActivation(int(num_init_features / 2), num_init_features, 3, 1, 1)
        self.stem_2p = nn.MaxPool2d(kernel_size=3, stride=1,padding=1)
        self.stem_3 = ConvBNActivation(num_init_features * 2, num_init_features, 1, 1, 0)
        
        #self.shortcut = ConvBNActivation(inp, num_init_features, 1, 1, 0)

    def forward(self, x):
        stem_1_out = self.stem_1(x)
        b = stem_1_out
        stem_2b_out = self.stem_1_1(stem_1_out)
...
class StandardUnit(torch.nn.Module):
    def __init__(self,
        ...
        #self.use_projection = (in_channels != out_channels) and stride==1
        #self.stride = stride 
        ...
        if self.use_projection:
            ...
            if stride == 2:
              #self.projection = conv3x3_block(in_channels=in_channels, out_channels=out_channels, stride=stride, activation=None)
              self.projection=torch.nn.Sequential(nn.Conv2d(in_channels, in_channels, 3, stride=2,padding=0, groups =in_channels,bias=False),
...
class PreactUnit(torch.nn.Module):
    def __init__(self,
        ...
        if self.use_projection:
            ...
            if stride == 2:
              self.projection = conv3x3_block(in_channels=in_channels, out_channels=out_channels, stride=stride, use_bn=False, activation=None)
...
class ResNet(torch.nn.Module):
    def __init__(self,
        ...
        if self.use_init_unit_large:
            ...
            self.backbone.add_module("init_unit", StemBlock(in_channels, init_unit_channels))
